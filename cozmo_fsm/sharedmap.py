...
from .worldmap import RobotForeignObj, LightCubeForeignObj, WallObj, MarkerObj
        ...
        while(True):
            for key, value in self.robot.world.world_map.objects.items():
                if isinstance(key,LightCube):
                    self.to_send["LightCubeForeignObj-"+str(value.id)]= LightCubeForeignObj(id=value.x, x=value.x, y=value.y, z=value.z, theta=value.theta)
